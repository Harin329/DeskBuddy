...
IN `requestType` VARCHAR(50), IN `forwardLocation` VARCHAR(50), IN `additionalInstructions` VARCHAR(500), IN `reqCompletionDate` DATE, IN `in_status` VARCHAR(50))
BEGIN
    ...
    VALUES (`mailID`, `employeeID`, `employeeName`, `employeeEmail`, `employeePhone`, `requestType`, `forwardLocation`,
            `additionalInstructions`, `reqCompletionDate`, null, `in_status`, null, null, now());
END;
...
SET `employee_phone` = employeePhone,`request_type` = `requestType`, `forward_location` = `forwardLocation`, `additional_instructions` = `additionalInstructions`,
    `req_completion_date` = `reqCompletionDate`, `status` = `in_status`, `modified_at` = now()
WHERE `mail_id` = `mailID` AND `employee_id` = `employeeID`;
END;

CREATE PROCEDURE `updateRequestAdmin` (IN `mailID` INT, IN `in_status` VARCHAR(50), IN `adminEid` VARCHAR(50), IN `in_response` VARCHAR(50))
BEGIN
UPDATE `mail_request`
SET `status` = `in_status`, `admin_eid` = `adminEid`, `response` = `in_response`, `modified_at` = now()
