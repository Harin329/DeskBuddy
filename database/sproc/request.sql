...
CREATE PROCEDURE `updateRequestAdmin` (IN `mailID` INT, IN `in_status` VARCHAR(50), IN `adminEid` VARCHAR(50), IN `in_response` VARCHAR(50), IN `modifiedAt` DATE)
BEGIN
UPDATE `mail_request`
SET `status` = `in_status`, `admin_eid` = `adminEid`, `response` = `in_response`, `modified_at` = `modifiedAt`
